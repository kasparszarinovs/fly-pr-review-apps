...
ignorefile="$INPUT_DOCKER_IGNOREFILE"
...
if ! flyctl status --app "$app"; then
  ...
  flyctl deploy --dockerfile "$dockerfile" --ignorefile "$ignorefile" --config "$config" --app "$app" --image "$image" --region "$region" --strategy immediate 
fi
